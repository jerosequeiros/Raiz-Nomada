...
function addItemCarrito(newItem) {
  ...
  for (let i = 0; i < carrito.length; i++) {
    if (carrito[i].title.trim() === newItem.title.trim()) carrito[i].cantidad++;
...
function sumarprecio(precio){
  Total = Total + precio;
   document.getElementById("itemCartTotal").innerText = "TOTAL: $" + Total

}
function Borrar() {
  Total=0
  document.getElementById("elTotal").innerHTML = "The current total is: " + "$" + 0;
}
